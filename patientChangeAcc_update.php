...
require_once "connection.php";
    ...
    if(isset($_POST['action']) && $_POST['action'] == 'updateCredentials'){
        ...
        $patientCurrentPassword = $_POST['patientCurrentPassword'];
        ...
        if($patientCurrentPassword == $inputCurrentPassword){
            ...
            // if(isset($_POST['submit'])){
                ...
                if($newPassword == $conPassword){
                    $stmt_u = $conn->prepare("UPDATE users SET pass_word=? WHERE patientId=?");
                    $stmt_u->bind_param("ss", $newPassword, $patientId);
                    $stmt_u->execute();
